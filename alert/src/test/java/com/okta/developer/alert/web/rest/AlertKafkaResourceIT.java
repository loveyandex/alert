...
class AlertKafkaResourceIT {
    ...
    void setup() {
        ...
        AlertKafkaResource kafkaResource = new AlertKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/alert-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/alert-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
