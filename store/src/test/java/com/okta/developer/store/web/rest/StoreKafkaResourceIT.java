...
class StoreKafkaResourceIT {
    ...
    void setup() {
        ...
        StoreKafkaResource kafkaResource = new StoreKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/store-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/store-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
